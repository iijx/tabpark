...
const webClick = (webItem: WebSite) => window.open(webItem.url);

const title = computed(() => {
  return props.item.title || (/^[^|\-/｜•—]+/.exec(props.item.title) || [])[0] || props.item.title || '';
});

const icon = computed(() => 'https://cdn.pastecuts.cn/web-icons/' + props.item?.iconSrc || `${props.item?.url}/favicon.ico || ''`);
